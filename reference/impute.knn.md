...
- rowmax:
  ...
  A numeric value (default = 0.5) specifying the maximum percent missing
  data allowed in any row. For any rows with more than `rowmax`\*100%
  missing are imputed using the overall mean per sample.

- colmax:

  A numeric value (default = 0.8) specifying the maximum percent missing
  data allowed in any column. If any column has more than `colmax`\*100%
