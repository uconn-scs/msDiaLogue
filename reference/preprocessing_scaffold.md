...
- zeroNA:
  ...
  A logical value (default = TRUE) specifying whether 0's should be
  converted to NA's.

- oneNA:

  A logical value (default = TRUE) specifying whether 1's should be
  converted to NA's.

## Value

- If the data contains columns for Gene Ontology (GO) annotation terms,
  the function returns a list containing both a preprocessed data frame
  and a list of GO terms.

- If no GO annotation columns are present, the function returns only a
