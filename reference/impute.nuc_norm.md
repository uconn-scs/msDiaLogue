...
- rank.max:
  ...
  default is set to one less than the minimum dimension of the data set.

- lambda:

  A numeric value specifying the nuclear-norm regularization parameter.
  If `lambda = 0`, the algorithm convergence is typically slower. The
  default is set to the maximum singular value obtained from the
  singular value decomposition (SVD) of the data set.

- thresh:

  A numeric value (default = 1e-5) specifying the convergence threshold,
...
- final.svd:
  ...
  A logical value (default = TRUE) specifying whether to perform a
  one-step unregularized iteration at the final iteration, followed by
