...
This script is to allow [PMF](https://proteomics.uconn.edu/) users to apply
msDiaLogue functions to proteomics results provided in Scaffold 5.
The msDiaLogue was originally developed to provide an R-based analysis workflow
for DIA data searched in Spectronaut (which could not be uploaded into
Scaffold 5 and thus left [PMF](https://proteomics.uconn.edu/) users with results
in the form of crashingly large Excel files before msDiaLogue became available).
The msDiaLogue initial step, [Preprocessing](preprocessing.html), was built for
a Spectronaut report format. This expansion gives msDiaLogue the capability to
...
Analysis --> Other Settings, Quantitative Method dropdown).
[PMF](https://proteomics.uconn.edu/) recommends the Average Precursor Intensity
value.
...
msDiaLogue `preprocessing_scaffold()` knows to treat these as NA via
`zeroNA = TRUE` and `oneNA = TRUE`. If normalization is applied in Scaffold,
non-quantifiable abundances are transformed into fractional values which will
not be converted properly in `preprocessing` and your data will no longer yield
...
You can also fix this in Excel after exporting, before you import into R,
...
1-peptide identifications included, msDiaLogue will not stop you. However,
the Scaffold report does not provide information about how many peptides/protein
were identified, so unlike with a Spectronaut-based report, you cannot filter
based on this information once in msDiaLogue. </div>
...
pick up at the `transformation()` step in the msDiaLogue script as provided
in the main [Transformation](transformation.html) page.


<div class="note">
**Note:** You cannot use the function `preprocessing()` in the main template,
as that is designed for inputs in a Spectronaut report format. The function for
Scaffold data is called `preprocessing_scaffold()`, given in the code box below. </div>
