...
For example, the proteins named "ALBU_BOVIN" and those containing "HUMAN" are chosen to
be filtered out.
...
("ALBU_BOVIN" + "\*HUMAN") will be saved as a .csv file named *filtered_out_data.csv* in
...
Alternatively, if we would to keep proteins like "ALBU_BOVIN" and "\*HUMAN", simply
...
match with "Putative zinc finger protein 840" or "Bovine serum albumin" will be kept.
...
Oh! The message "Warning: Removed 55 rows containing non-finite values" indicates the
presence of 55 NA (Not Available) values in the data. These NA values arise when a protein
...
samples for that protein. For instance, the label "ZN840_HUMAN (8\%)" indicates that,
within all observations for the protein "ZN840_HUMAN", 8\% of the data is missing.
Additionally, the percentage in the legend represents the proportion of missing data in
the whole dataset. In this case, 10.2\% of the data in `dataNorm` is missing.
...
For example, in the case of the protein "ZN840_HUMAN," there are 1 NA values in the
samples, representing 8.33\% of the missing data for "ZN840_HUMAN" within that sample and
1.82\% of the total missing data in the entire dataset.
...
In the case of `dataImput`, one protein, namely "ALBU_BOVIN", has constant values, leading
to the error message. We choose to remove this protein in the PCA.
...
This plot reveals that 42 proteins are shared by 50pmol, 100pmol, and 200pmol, while only
3 proteins are shared by 100 pmol and 200pmol, but not with 50pmol.
...
where `saveVenn = TRUE` refers to the data containing logical columns representing sets in
Venn plot information will be saved as a .csv file named *Venn_information.csv* in the
current working directory.

In the example above, 50pmol, 100pmol, and 200pmol groups share 42 proteins. Notably, 3
proteins are exclusively found in the 100pmol and 200pmol groups.
...
The function `pullProteinPath()` allows you to see the values associated with specific
proteins that match exactly and/or proteins that match the regular expression pattern
at each step of processing. This can be useful for questions such as, "Were all of
...
used to check whether some given proteins' fold-change might have been a processing artifact.
