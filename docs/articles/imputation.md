...
After the above filtering, any entries that did not pass the percent
present threshold will still have NA values and will need to be filtered
out. And `saveRm = TRUE` indicates that the filtered data will be saved
as a .csv file named *filtered_NA_data.csv* in the current working
directory.
